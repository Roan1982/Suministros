  ...
  if (ordenSelect) {
    ...
    if (window.jQuery && window.jQuery.fn.select2) {
      ...
      window.jQuery(ordenSelect).on('select2:clear', function() {
        ...
        // Actualizar bienes disponibles
        setTimeout(() => {
          console.log('[DEBUG] Select2 orden clear, restaurando opciones...');
          restaurarOpcionesBienes();
        }, 100);
      });
    }
...
// Función para validar duplicados y stock agotado - VERSION 2 - GLOBAL
function validarCombinacionesDuplicadas() {
  console.log('[DEBUG VERSION 2] Validando combinaciones duplicadas...');
  
  const filas = document.querySelectorAll('#items-table-body tr.form-row');
  console.log('[DEBUG VERSION 2] Filas encontradas:', filas.length);
  
  const combinaciones = new Map(); // key: "bienId-ordenId", value: {cantidad_total, filas[]}
  
  // Primero, recolectar todas las combinaciones bien-orden con sus cantidades
  filas.forEach((fila, index) => {
    ...
    console.log('[DEBUG VERSION 2] Fila', index, '- Bien:', bienSelect?.value, 'Orden:', ordenSelect?.value, 'Cantidad:', cantidadInput?.value);
    
    if (bienSelect?.value && ordenSelect?.value && cantidadInput?.value) {
      const key = `${bienSelect.value}-${ordenSelect.value}`;
      const cantidad = parseInt(cantidadInput.value) || 0;
      ...
      if (cantidad > 0) {
        ...
        const combo = combinaciones.get(key);
        combo.cantidad_total += cantidad;
        combo.filas.push({fila, index, cantidad});
        
        // Obtener el stock disponible desde la celda de stock
        const stockCell = fila.querySelector('.stock-cell');
        if (stockCell && stockCell.textContent !== '-') {
          combo.stock_disponible = parseInt(stockCell.textContent) || 0;
          console.log('[DEBUG VERSION 2] Stock disponible para', key, ':', combo.stock_disponible);
        }
      }
    }
  });
  
  console.log('[DEBUG VERSION 2] Combinaciones encontradas:', Array.from(combinaciones.entries()));
  
  // Verificar qué combinaciones agotan completamente el stock
  const combinacionesQueAgotanStock = new Set();
  
  combinaciones.forEach((combo, key) => {
    console.log('[DEBUG VERSION 2] Evaluando combinación', key, '- Cantidad total:', combo.cantidad_total, 'Stock disponible:', combo.stock_disponible);
    if (combo.cantidad_total >= combo.stock_disponible && combo.stock_disponible > 0) {
      console.log('[DEBUG VERSION 2] *** COMBINACIÓN QUE AGOTA STOCK:', key, combo);
      combinacionesQueAgotanStock.add(key);
    }
  });
  
  console.log('[DEBUG VERSION 2] Total combinaciones que agotan stock:', combinacionesQueAgotanStock.size);
...
window.validarCombinacionesDuplicadas = validarCombinacionesDuplicadas;
});
